...
sequenceDiagram
    ...
    activate Sender Wallet
    Note over Sender,Sender BUX: .... ℹ️ Preparation (unlocking) of the inputs for the transactions is presented in the separate diagram...
    ...
    Note right of Sender Wallet: SharedSec = masterPrivateKey * recPub
...
sequenceDiagram
    ...
    loop Every UTXO received from confirmed contact
        Sender Wallet ->> Sender Wallet: Calculate shared secret (SharedSec) 
        Note right of Sender Wallet: SharedSec = masterPrivateKey * contact public key
        ...
        Note right of Sender Wallet: dk[UTXO] = (masterPrivateKey + hn) % N <br/> where N is an order of the elliptic curve
