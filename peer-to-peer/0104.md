...
- **Excluded**:
  - Non-whitelisted headers (like `x-bsv-auth-*`, `content-type`, `accept`)  
  - Cookies, user-agent, etc.  

The **order** of included headers **MUST** be consistent (the order they appear over the wire) to ensure matching preimages on both sides.
...
### 6.8 Signature Preimage Calculation for HTTP Messages

The code references BRC-103’s standard approach: concatenating these fields in a stable order:

1. `x-bsv-auth-request-id` (32 bytes, base64-decoded, if relevant).  
