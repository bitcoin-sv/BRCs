...
The signature for a certificate is over all the fields, the subject, the certifier, the type, the serial number, the revocation outpoint, and the validation key. Each field is organized determanistically, in alphabetical order, in a JSON object. We stipulate the use of the (default) [Stable Stringify Algorithm](https://www.npmjs.com/package/json-stable-stringify) to avoid ambiguous ordering.
...
If the certificate's revocation outpoint ever becomes spent, we specify that the certificate should be treated as invalid.
