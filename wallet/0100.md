...
- The wallet interface integrates transaction validation protocols that follow [BRC-67](../transactions/0067.md)'s SPV method. By validating UTXOs through matching `txid` and proof inclusion within the blockchain, wallets can quickly confirm that tokens are genuine without needing the entire chain.
...
- Must not start with `p ` (allows for future "specially permissioned" protocols, as specified in [BRC-98](../wallet/0098.md))
...
- Must not start with `p ` (allows for future "specially permissioned" baskets, as specified in [BRC-99](../wallet/0099.md))
...
/**
 * @typedef {Byte[]} BEEF
 * An array of integers, each ranging from 0 to 255, indicating transaction data in BEEF ([BRC-62](../transactions/0062.md)) format.
 * @remarks
 ...
 * An array of integers, each ranging from 0 to 255, indicating transaction data in Atomic BEEF ([BRC-95](../transactions/0095.md)) format.
 * @remarks
 * Validate at runtime to ensure each element is within the specified range.
 */
