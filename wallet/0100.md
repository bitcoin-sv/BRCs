...
At the heart of the Wallet Interface lies the **[BRC-42](../key-derivation/0042.md): BSV Key Derivation Scheme**. This scheme defines how keys are derived between two interacting parties. BKDS leverages the secp256k1 elliptic curve and enables participants to derive multiple unique public-private key pairs from a shared master key.
...
- The shared secret is then used to generate a scalar through HMAC [BRC-56](../wallet/0056.md#hmacs) and convert it to a point on the elliptic curve.
...
- **Internalization**: `internalizeAction` enables wallets to accept and manage incoming transactions by parsing, tagging, and organizing outputs into baskets, as outlined in [BRC-46](../wallet/0046.md).
...
- **Relinquishment:** `relinquishOutput` releases an output from a basket tracked by the wallet, even if it has yet to be spent, adhering to the basket permissioning rules outlined in [BRC-99](../wallet/0099.md).

### Public Key Management:

- **Key Retrieval**: `getPublicKey` Key Retrieval: getPublicKey retrieves public keys derived using BKDS, as specified in [BRC-42](../key-derivation/0042.md), and identity keys, adhering to the permissioning standards outlined in [BRC-43](../key-derivation/0043.md).
...
- **Encryption/Decryption**: `encrypt` and `decrypt` methods implement secure encryption and decryption of data using derived keys, as defined in [BRC-2](../wallet/0002.md), and protocol definitions outlined in [BRC-43](../key-derivation/0043.md), enabling private exchanges of information between counterparties.
- **HMAC Operations**: `createHmac` and `verifyHmac` allow for the creation and verification of Hash-based Message Authentication Codes (HMAC) to ensure data integrity as per [BRC-56](../wallet/0056.md#hmacs).
- **Signatures**: `createSignature` and `verifySignature` enable the creation and verification of digital signatures, both public and private, essential for validating the authenticity of transactions, documents, and data as specified in [BRC-3](../wallet/0003.md).

### Identity and Certificate Management:

- **Certificate Acquisition**: `acquireCertificate` allows the wallet to obtain identity certificates, either by directly saving them or through a standardized issuance protocol. Conversely, `relinquishCertificate` allows an old certificate to be removed, following the lifecycle management principles of [BRC-52](../peer-to-peer/0052.md).
- **Certificate Listing and Discovery**: `listCertificates`, `discoverByIdentityKey`, and `discoverByAttributes` enable querying of identity certificates owned by the user or others based on identity keys or specific attributes, as outlined in [BRC-52](../peer-to-peer/0052.md).
- **Proving Identity Certificates**: `proveCertificate` Proving Identity Certificates: proveCertificate leverages selective revelation protocols defined in [BRC-52](../peer-to-peer/0052.md) while integrating future-proof proof schemes from [BRC-97](../wallet/0097.md). This provides enhanced flexibility and enables users to securely prove their identity or certified attributes to third parties when required.
...
- `SatoshiValue`: Represents a value in Satoshis, ranging between 1 and 2.1 \* 10^15, consistent with tokenized UTXO values defined in [BRC-45](../tokens/0045.md).
...
- `context`: (Optional) Additional contextual data relevant to the error---often binary or debug information.
  When errors occur, they must be communicated and thrown in accordance with the transport protocols outlined in [BRC-43](../key-derivation/0043.md), ensuring that all structural elements are preserved and properly conveyed. For errors specifically related to cryptographic operations, such as encryption or decryption, wallets must adhere to the standardized error structure defined in [BRC-2](../wallet/0002.md). Specific instantiations or realizations of this interface, comprising APIs or transport mechanisms for messages between wallets and applications, must specify how these errors are communicated.
...
The Wallet ABI defines a binary protocol for communication between an application and the user's digital wallet, adhering to the transport protocol rules in [BRC-43](../key-derivation/0043.md). Each message transmitted over the Wallet Wire consists of a structured binary frame that includes the method call code, originator information, parameters, and return values. The protocol is designed to be efficient, minimizing the data transmitted while ensuring all necessary information is accurately conveyed with secure communication.
...
The originator field, serialized as per [BRC-43](../key-derivation/0043.md), is the fully qualified domain name (FQDN) of the application making the request. It is included in each message to allow the wallet to:
...
Errors are communicated using a structured format, ensuring consistent and detailed information about any issues that occur during method execution. This format adheres to the definitions in [BRC-43](../key-derivation/0043.md), enabling precise error reporting, seamless debugging, and interoperability across implementations.
...
**Action**: A transaction within the Wallet Interface that includes additional metadata, such as a description, label, or custom instruction. While standard blockchain transactions focus solely on inputs and outputs, Actions extend this functionality to support application-specific use cases, improving organization and interoperability by including the outlined metadata, facilitating richer interactions in wallet-to-application communication.
Example Usage:
The **createAction** method generates a new Action, which can then be signed and broadcast as a **Transaction**. The additional metadata helps categorize and process these transactions within applications.
...
**seekPermission**: A parameter in the Wallet Interface methods that determines whether user consent is required for the action.
`Value 1`: Explicit user consent is sought before proceeding.
`Value 0`: Consent is not required for the operation.
`Value -1`: Indicates the absence of this parameter in the method call.
Example Usage: Used in methods like `listActions` or `internalizeAction` to decide if explicit user authorization is needed during execution.
