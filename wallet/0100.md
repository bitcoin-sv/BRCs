...
**SECTION TL;DR**: We use the BSV Blockchain. We use the secp256k1 elliptic curve. We use compressed, DER-formatted public keys. We use BKDS key derivation. We use [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md) for security levels, protocol IDs, key IDs, and counterparties (with [BRC-44](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0044.md) protocols reserved for internal wallet use). We subscribe to the [BRC-45](https://github.com/bitcoin-sv/BRCs/blob/master/tokens/0045.md) idea that "Outputs are tokens". We use [BRC-46](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0046.md) output baskets for tracking tokens. For categorization and filtering purposes, we allow transactions to be given a set of labels, and outputs to be given a set of tags. We utilize the [BRC-67](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0067.md) rules for SPV validation. We use the BEEF standard outlined in [BRC-62](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0062.md) for representing transactions. For encryption and decryption, we use the methods described in [BRC-2](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0002.md). For creating digital signatures, we use the methods described in [BRC-3](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0003.md). For HMACs, we derive symmetric keys as in [BRC-2](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0002.md), but then use them for HMAC operations instead of AES-GCM encryption. For the digital certificate structure and field encryption scheme, we use [BRC-52](https://github.com/bitcoin-sv/BRCs/blob/master/peer-to-peer/0052.md). For internalizing payment outputs that increase the user's wallet balance, we employ the key derivation protocol described within [BRC-29](https://github.com/bitcoin-sv/BRCs/blob/master/payments/0029.md). For revealing key linkages, we employ the two methods described within [BRC-69](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0069.md), and we protect this information as described in [BRC-72](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0072.md). These foundational prerequisites allow us to fully define and specify the behavior and functionality of the digital wallet system.
...
[BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md) plays a vital role in organizing how keys are used and accessed in the wallet. This standard introduces **Security Levels, Protocol IDs, Key IDs, and Counterparties** rules that govern key derivation, permissions, and data access within standard wallets.
...
[BRC-45](https://github.com/bitcoin-sv/BRCs/blob/master/tokens/0045.md) defines **UTXOs as Tokens**, asserting that Unspent Transaction Outputs (UTXOs) are the base units of tokenization within Bitcoin. This principle is integral to the interface, where UTXOs serve as tokens that can be managed within wallets using custom baskets, as discussed below.
...
- The wallet interface integrates transaction validation protocols that follow [BRC-67](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0067.md)'s SPV method. By validating UTXOs through matching `txid` and proof inclusion within the blockchain, wallets can quickly confirm that tokens are genuine without needing the entire chain.

### 5. **Tracking and Managing Outputs with BRC-46**

To facilitate complex tracking and interaction with UTXOs, [BRC-46](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0046.md) establishes **Wallet Transaction Output Tracking (Output Baskets)** within the interface.
...
- Like with key derivation, permissioning is enforced for applications executing operations involving baskets. The security model here follows from [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md), ensuring consistency across the system.
...
- The BEEF format specified in [BRC-62](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0062.md) is optimized for SPV and designed for efficient data transmission, focusing on economy of information while retaining verification integrity.
...
- [BRC-67](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0067.md) outlines the steps to be undertaken for verifying a transaction, including script validation, fee checking, sequence, and locktime examination. These checks ensure that transactions processed through the wallet interface are legitimate and contextually accurate.
...
Security is paramount in the Wallet Interface, and [BRC-2](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0002.md) defines **Encryption and Decryption** operations encapsulated within the interface's cryptographic functionality.
...
- [BRC-3](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0003.md) mandates use of ECDSA over secp256k1 keys.
- Derived child keys (from BKDS) are used to sign data based on the security levels, protocol IDs, and key IDs defined in [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md).
...
- The signature is verifiable by the recipient using derived public keys. The recipient uses [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md) mechanics to derive the corresponding public key and validates the signature using this key over the provided data.

### 9. **Identity Certificates and Selective Revelation with BRC-52**

[BRC-52](https://github.com/bitcoin-sv/BRCs/blob/master/peer-to-peer/0052.md) defines **Identity Certificates**, which incorporate selective revelation protocols that wallets must support.
...
- **SPV Verification:** Regardless of whether a payment increments a user's wallet balance via [BRC-29](https://github.com/bitcoin-sv/BRCs/blob/master/payments/0029.md), inserts outputs into custom baskets with tags and custom instructions, or both, the wallet always employs [BRC-67](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0067.md) to confirm the transaction's validity according to the rules of SPV.

**Flexible Payment Handling:**

- **Multiple Outputs and Transactions:** [BRC-29](https://github.com/bitcoin-sv/BRCs/blob/master/payments/0029.md) supports handling multiple outputs and even transactions within a single "payment." Each transaction output can be individually indexed with a unique derivation suffix, allowing the wallet to differentiate and manage multiple outputs efficiently, while all outputs within one given payment share a common derivation prefix.
...
- **BKDS Based Key Linkage:** [BRC-69](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0069.md) outlines two methods for revealing key linkages from BKDS derived keys. The first method allows wallets to reveal a root ECDH shared secret between a user's identity key and another counterparty's key, enabling anyone to link all interactions between them. The second method reveals the specific key offset for individual derived child keys, enabling audit trails while preserving privacy in other contexts.

**Protection of Linkage Information:**

- **BRC-72 Integration:** Protecting sensitive linkage data is paramount. [BRC-72](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0072.md) specifies mechanisms for encrypting key linkage revelations when they are in transit. This encryption is done using the [BRC-2](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0002.md) AES-256-GCM method, ensuring that only authorized verifiers (recipients) can decrypt and access the linkage data. This preserves privacy and security, even when key linkages must be revealed for audit or verification purposes.

- Additionally, [BRC-94](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0094.md) proofs help overcome the limitations described in [BRC-93](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0093.md). BRC-97 allows for future zero-knowledge proof types to be used in the context of specific key linkage revelation as technology develops.
...
- Must not start with `p ` (allows for future "specially permissioned" protocols, not contemplated by this specification, see BRC-98)
...
- Must not start with `p ` (allows for future "specially permissioned" baskets, not contemplated by this specification, see BRC-99)
...
**AES-256-GCM**: Advanced Encryption Standard (AES) cipher with a 256-bit key size using Galois/Counter Mode (GCM); used for symmetric encryption and decryption operations within the Wallet Interface (as per [BRC-2](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0002.md)). By convention, 32-byte initialization vectors are prepended to the beginning of the ciphertext.
...
**Background Evaluation Extended Format (BEEF)**: A compact data format specified in [BRC-62](https://github.com/bitcoin-sv/BRCs/blob/master/transactions/0062.md) for representing Bitcoin transactions optimized for Simplified Payment Verification (SPV) and efficient data transmission within the Wallet Interface.

**Baskets**: Conceptual containers within a wallet used to group and manage specific Unspent Transaction Outputs (UTXOs) as per [BRC-46](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0046.md), enabling organized tracking and handling across applications or protocols.
...
**BKDS (BSV Key Derivation Scheme)**: A key derivation scheme defined in [BRC-42](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0042.md) that allows wallets to derive multiple unique public-private key pairs from a shared master key using the secp256k1 elliptic curve and a counterparty.
...
**Certificate**: In the context of [BRC-52](https://github.com/bitcoin-sv/BRCs/blob/master/peer-to-peer/0052.md), a digital identity document that encapsulates a subject's identity information, certified by a trusted entity, with support for selective field encryption.
...
**Counterparty**: An entity (e.g., sender, receiver, verifier) involved in a transaction or key derivation process, identified by their public key; used in BKDS and defined in [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md).
...
**Derivation Prefix/Suffix**: Values used during the internalization of payment outputs, as described within [BRC-29](https://github.com/bitcoin-sv/BRCs/blob/master/payments/0029.md) to generate different key pairs for each output across the same payment.
...
**Digital Signature**: A cryptographic value generated using a private key that verifies the authenticity and integrity of data, as outlined in [BRC-3](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0003.md) with ECDSA.
...
**Encryption/Decryption**: The processes of scrambling data to prevent unauthorized access (encryption) and restoring it to its original form (decryption), as specified in [BRC-2](https://github.com/bitcoin-sv/BRCs/blob/master/wallet/0002.md) with AES-256-GCM and used within the Wallet Interface.
...
**Key ID**: A unique identifier differentiating specific keys under the same Protocol ID, allowing for multiple derived keys with different purposes within the same context as defined in [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md).

**Key Linkage**: Information that reveals the relationship between derived keys, used for transparency, auditability, and verification, particularly as per [BRC-69](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0069.md) and protected during transit by [BRC-72](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0072.md).
...
**Payment Internalization**: The process by which a wallet accepts and manages incoming transactions by parsing, tagging, and organizing outputs, as per [BRC-29](https://github.com/bitcoin-sv/BRCs/blob/master/payments/0029.md).
...
**Protocol ID**: An identifier used in [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md) to define the context or usage of a derived key, combining a security level and a protocol string (e.g., `[1, "document signing"]`).
...
**Security Level**: In [BRC-43](https://github.com/bitcoin-sv/BRCs/blob/master/key-derivation/0043.md), a classification that determines the required permissions and access controls for using a derived key (Level 0: open access, Level 1: requires cross-counterparty authorization, Level 2: requires individual permission for each counterparty).
