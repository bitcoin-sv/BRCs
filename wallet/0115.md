# BRC-115: Wallet Permissions and Counterparty Trust (PACT)

Brayden Langley (brayden@projectbabbage.com)

Stephen         (stephen@projectbabbage.com)

Jackie Lu       (jackie@projectbabbage.com)

## Abstract

This document defines the wallet permissions system used by Metanet-compatible wallets and applications.

Its goals are to:
- Clearly specify how applications request permissions
- Define how permissions are scoped, granted, denied, and enforced
- Distinguish **group permissions** from **counterparty trust (PACT)**
- Provide an authoritative reference for wallet behavior and application integration

This document is **normative**: it describes the expected behavior of compliant wallets and applications, independent of UI or implementation language.
...
- **Security Level**: An integer (0, 1, or 2) that classifies protocol access. **Level 0** is open (no permission required). **Level 1** protocols are application-scoped (the app itself is the counterparty). **Level 2** protocols are counterparty-specific (a distinct external entity is involved).
...
The wallet resolves originator identity from HTTP request headers (`Origin` or `Originator`) during the authentication handshake.
...
3. **Counterparty permission declarations** - Under `metanet.counterpartyPermissions`, applications declare which Level 2 protocols require peer trust (PACT). This tells the wallet which protocols to include in the PACT prompt when an untrusted counterparty is encountered.
...
| `protocolPermissions` | array | Protocol access requests (Level 1 or 2) |
...
When the wallet encounters a permission-requiring operation and `seekGroupedPermission` is enabled, it:
1. Fetches the originator's manifest.
2. Checks whether the current operation is covered by the manifest's `groupPermissions`.
3. Filters out permissions already granted (by looking up existing on-chain tokens).
4. Presents all remaining permissions as a single grouped prompt.
5. After the user responds, checks whether the specific triggering operation is now satisfied.

#### Counterparty Permissions Declaration

The `metanet.counterpartyPermissions` object declares PACT requirements - the Level 2 protocols that require explicit counterparty trust before being granted:

| Field | Type | Description |
|-------|------|-------------|
| `description` | string? | Human-readable description |
| `protocols` | array | Array of `{ protocolID: [2, name], description }` - **Level 2 only** |

The wallet validates this structure via `validateCounterpartyPermissions()`, which enforces that all declared protocols have `protocolID[0] === 2`. Entries that are not Level 2 are silently dropped.

Including `counterpartyPermissions` tells the wallet: "when this app encounters a new counterparty, ask the user to trust them for these protocols before proceeding." Without it, each Level 2 protocol permission is requested individually as it's used.
...
```json
{
  ...
  "metanet": {
    ...
    "groupPermissions": {
      ...
      "protocolPermissions": [
        {
          "protocolID": [2, "peer-messaging"],
          "counterparty": "",
          "description": "Send and receive encrypted messages"
        },
        {
          "protocolID": [2, "peer-presence"],
          "counterparty": "",
          "description": "Online status sharing"
        }
      ],
      "basketAccess": [
        {
...
- HTTPS is enforced for non-localhost domains; HTTP is permitted for `localhost` only.
- Requests are proxied through the wallet backend, which validates the URL scheme (HTTPS only) and path (must be `/manifest.json`).
- Manifest data is cached in memory with a 5-minute TTL. Concurrent fetches to the same originator are deduplicated.
- The desktop client additionally caches `name` and favicon in local storage per domain.
...
Each permission type has distinct scope, grant options, and prompting behavior. All four types support a **renewal** flag indicating the permission is being re-requested after a previous grant has expired or been revoked.
...
- **`counterparty`** (optional): A compressed public key identifying the other party. Relevant only for Level 2 protocols.

#### Security Levels

- **Level 0**: Open usage - no permission check is required.
- **Level 1**: The application itself is the implicit counterparty. Protocol keys are derived per-originator. No counterparty prompt is required. The wallet SHOULD present the scope as application-only (e.g. "only with this app").
...
In the current implementation, signing and encrypting prompt by default; HMAC does not.
...
Protocol permission grants are binary (grant or deny). There are no amount limits or ephemeral flags. The wallet MAY set an expiry (default: 0, meaning no expiry).
...
```
{
  ...
  counterparty?: string       // Defaults to 'self' for Level 1
...
Note: The desktop client UI additionally surfaces `transactionAmount`, `totalPastSpending`, `amountPreviouslyAuthorized`, and `authorizationAmount` derived from the token lookup. The core request from the wallet-toolbox contains only `spending.satoshis` and `spending.lineItems`.

---

### 4.3 Basket Permissions

Basket permissions control an application's ability to interact with named data baskets - structured, wallet-managed collections of outputs.
...
```
{
  ...
  permissions: {
    description?: string
    protocols: Array<{
      protocolID: [number, string]  // [securityLevel, protocolName] - Level 2 only
      description: string
    }>
  }
}
...
The wallet presents this to the user as: "This person (counterparty) wants to interact with you through this app (originator) using these protocols."
...
- If the request is a **Level 2 protocol** with a specific counterparty, and the manifest declares `counterpartyPermissions` covering that protocol -> the wallet checks for an established PACT and, if missing, triggers a **PACT prompt** (`onCounterpartyPermissionRequested`).

- If the request is covered by the manifest's `groupPermissions` (any of the four permission types) -> the wallet filters out already-granted permissions and triggers a **grouped permission prompt** (`onGroupedPermissionRequested`).
...
- If the request is not covered by any manifest declaration, or if the manifest is unavailable -> the wallet triggers an **individual permission prompt** for the specific type (`onProtocolPermissionRequested`, `onBasketAccessRequested`, `onCertificateAccessRequested`, or `onSpendingAuthorizationRequested`).
...
The wallet-toolbox implements the following PACT lifecycle:

#### Establishing a PACT

`maybeRequestPact()` is called during `requestPermissionFlow()`. It triggers only when **all** of the following conditions are met:

1. `seekGroupedPermission` configuration is `true`
2. A counterparty permission handler is registered
...
2. Checks which declared protocols the counterparty already has tokens for.
3. Presents the missing protocols to the user via the `onCounterpartyPermissionRequested` callback.
4. On grant, creates on-chain protocol permission tokens for each approved protocol.
5. Marks the PACT as established in the in-memory cache.

#### Checking if a PACT Exists

`hasPactEstablished()` checks in this order:

1. If counterparty is `'self'` or `'anyone'` -> always `true`.
2. If the originator:counterparty pair is in the in-memory `pactEstablishedCache` -> `true`.
3. If the manifest has no `counterpartyPermissions` -> `true` (no PACT needed). This means applications that do not declare `counterpartyPermissions` will never trigger a PACT prompt - Level 2 protocol requests will fall through to individual or grouped permission prompts instead.
4. If the first protocol declared in `counterpartyPermissions` has an existing on-chain token for this counterparty -> `true` (and caches the result).
5. Otherwise -> `false`.

#### Request Deduplication

PACT requests use the key format `pact:${originator}:${counterparty}`. If a PACT request is already in flight for the same originator + counterparty, subsequent requests piggyback on the pending promise rather than creating a duplicate prompt.
...
Permissions are checked **at runtime** when an application calls a wallet API method. The `WalletPermissionsManager` wraps every BRC-100 wallet method (e.g. `encrypt`, `createAction`, `listOutputs`, `proveCertificate`) and intercepts the call to check permissions before forwarding to the underlying wallet.
...
6. The wallet checks the **in-memory permission cache** (5-minute TTL) and the **recent grant window** (15-second auto-allow for non-spending permissions).
7. The wallet looks up an **on-chain permission token** in the relevant admin basket.
8. If a valid, unexpired token is found -> the call is allowed and the result is cached.
9. If an expired token is found -> the wallet triggers a **renewal** flow (`renewal: true`, with the old token attached).
10. If no token is found -> the wallet triggers **`requestPermissionFlow()`**, which attempts four strategies in order:
    ...
    Each step returns `null` to signal "not handled, try next." The first non-null result is used.
...
If any condition fails, the grouped flow returns `null` and the request falls through to an individual prompt.

#### Inclusion Rules

The wallet determines whether a request is "included" in the manifest's group permissions based on the request type:

- **Protocol**: The request's `protocolID` must exactly match a manifest entry. Counterparty matching: if the manifest entry has `counterparty === ''` (empty string), it acts as a wildcard matching any counterparty. Otherwise, both must match. **Privileged** protocol requests are never included in grouped flows.
...
Only **one grouped permission flow** per originator is active at a time, enforced by a promise-chain lock (`withGroupedPermissionFlowLock()`). Concurrent requests for the same originator wait for the active flow to complete. When a waiting request enters after a grouped prompt has already run, it checks whether its permission was satisfied by the prior prompt before triggering a new one.

### 6.4 Peer-Grouped Requests

Peer-grouped requests are a specialized form of grouped request. They are tried **before** the full grouped flow (Step 3 in `requestPermissionFlow`) and **after** the PACT flow (Step 1).
...
2. A grouped permission UI handler is registered.
3. The current request is for a **protocol** permission.
4. The protocol is **Level 2**.

The wallet then:
1. Fetches the manifest's `groupPermissions`.
2. Filters to only Level 2 protocol entries matching the current request's counterparty (empty string counterparty in manifest acts as wildcard).
3. Checks if the current request's `protocolID` is among them.
4. Filters out already-granted permissions.
5. If any remain, presents them as a peer-grouped prompt (key: `group-peer:{originator}:{privileged}:{counterparty}`).
...
| 3 | securityLevel | `0`, `1`, or `2` |
| 4 | protocolName | Protocol identifier string |
| 5 | counterparty | `'self'` or public key |
...
- The default expiry for all grant operations is `0` (no expiry) unless explicitly specified.
- DSAP (spending) tokens always have `expiry = 0` - spending limits are enforced on a calendar month basis instead.

### 7.4 Granting

When the user approves a permission request, `grantPermission()`:

1. Resolves all pending promises for the request (multiple callers waiting on the same permission are all unblocked).
2. If `ephemeral !== true`, creates an on-chain token via `createPermissionOnChain()` (or renews via `renewPermissionOnChain()` if `renewal: true`).
3. Caches the permission in memory (5-minute TTL) and marks it in the recent-grant window (15-second auto-allow for non-spending).

Grant method variants:

- **`grantPermission({ requestID, expiry?, ephemeral?, amount? })`** - For individual permissions.
- **`grantGroupedPermission({ requestID, granted, expiry? })`** - For grouped permissions. `granted` is a partial `GroupedPermissions` containing the subset the user approved.
- **`grantCounterpartyPermission({ requestID, granted, expiry? })`** - For PACT. Creates protocol tokens for each approved Level 2 protocol.

**Ephemeral grants**: When `ephemeral: true`, no on-chain token is created. The permission is a one-time in-memory authorization. The caller's promise is resolved, but no persistent record is kept. Used primarily for one-off spending approvals.

### 7.5 Denial

When the user denies a permission:

- `denyPermission(requestID)` or `denyCounterpartyPermission(requestID)` is called.
- All pending promises for the request are **rejected** with an error: `'The user has denied the request for permission.'` (error code: `ERR_PERMISSION_DENIED`).
...
Granted permissions can be revoked by the user at any time through the wallet dashboard. Revocation works by **spending** the on-chain token output with no replacement - the BRC-100 transaction consumes the token UTXO and produces no new permission output.

The wallet supports:

- **Individual revocation**: `revokePermission(oldToken)` - spends and invalidates a single token.
- **Batch revocation**: `revokePermissions(oldTokens)` - revokes multiple tokens.
- **Full originator revocation**: `revokeAllForOriginator(originator, opts?)` - revokes all tokens for an application, optionally filtered by type.
...
3. On grant, `renewPermissionOnChain()` spends the old token and creates a new one with the updated expiry.
...
The wallet maintains two levels of caching to avoid repeated on-chain lookups:

| Cache | TTL | Purpose |
|-------|-----|---------|
| Permission cache | 5 minutes (`CACHE_TTL_MS`) | Caches confirmed permissions by request key |
| Recent grant window | 15 seconds (`RECENT_GRANT_COVER_MS`) | Auto-allows recently granted permissions (except spending) |
| Manifest cache | 5 minutes (`MANIFEST_CACHE_TTL_MS`) | Caches fetched manifest data per originator |
| PACT established cache | In-memory (no TTL) | Caches established PACT relationships |

### 7.9 Request Deduplication

When multiple API calls from the same originator require the same permission simultaneously, the wallet deduplicates them.

#### Request Key Format

A unique key is built for each request based on type, originator, scope fields, and usage type:

| Type | Key Format |
|------|-----------|
| Protocol | `proto:{originator}:{privileged}:{securityLevel},{protocolName}:{counterparty}:{usageType}` |
| Basket | `basket:{originator}:{basketName}:{usageType}` |
| Certificate | `cert:{originator}:{privileged}:{verifier}:{certType}:{field1\|field2\|...}:{usageType}` |
| Spending | `spend:{originator}:{satoshis}` |

Grouped flows use distinct key formats: `group:{originator}`, `group-peer:{originator}:{privileged}:{counterparty}`, and `pact:{originator}:{counterparty}`.

Note: Because the key includes `usageType`, two requests for the same protocol from the same originator but with different usage types (e.g. `signing` vs `encrypting`) produce different keys and are **not** deduplicated against each other, even though they need the same on-chain token.

#### Deduplication Mechanics

Deduplication occurs at two checkpoints within `requestPermissionFlow()`:

1. **Before grouped flows** - if a request with the same key is already active, the caller piggybacks on the existing promise.
2. **After grouped flows** - checked again in case another caller created an individual request during the grouped flow wait.

If no existing request matches, a new entry is created in the active requests map and a UI prompt is fired.

#### Resolution and Race Window

When the user grants an **individual** permission:
1. All piggybacked callers are resolved immediately.
2. The active request entry is deleted.
3. The on-chain token is created (async).
4. The permission cache and recent-grant window are populated.

Because the entry is deleted (step 2) **before** the token is created and cached (steps 3-4), there is a brief race window: a new request arriving between steps 2 and 4 will find no active request, no cache entry, and potentially no on-chain token yet, resulting in a duplicate prompt.

For **grouped** permission grants, the race window is smaller: tokens are created **before** promises are resolved and the entry is deleted (in a `finally` block).

This race window explains why concurrent `createAction` calls for the same permission may produce duplicate prompts, as observed in testing.
...
Wallets SHOULD:
- Cache manifest data per originator with a reasonable TTL (currently 5 minutes)
...
- On-chain permission token fields MUST be encrypted so that only the wallet owner can read them. The current implementation uses protocol `[2, 'admin permission token encryption']` for this purpose.
...
**Desktop Client (`metanet-client-desktop`):**
- Permission request queues are managed per type (protocol, spending, basket, certificate, counterparty, group) in React state. Each queue advances independently.
- The desktop client dispatches custom DOM events when permissions change (`protocol-permissions-changed`, `basket-access-changed`, `cert-access-changed`, `spending-authorization-changed`), enabling reactive UI updates.
- Permission lists use in-memory caching with signature-based change detection to avoid redundant refreshes.
- The desktop client adds UI-level queuing for PACT requests (deferring them during active group permission phases) as an additional layer on top of the toolbox's flow ordering.

**Verified Runtime Behaviors:**

The following behaviors have been verified against `peergroup.bsvapp.net` (a reference test application using the legacy `babbage` namespace - behavior is identical with `metanet`):

- **Level 1 vs Level 2 scope presentation**: Level 1 protocol prompts display scope as "only with this app" with counterparty shown as "anyone". Level 2 prompts display "only with this app and counterparty" with the full 66-character compressed public key.
- **Spending prompt UX**: Spending prompts display an itemized table (description + amount per line, including network fee and total) with three buttons: "Deny", "Allow up to N satoshis" (standing authorization), and "Spend" (ephemeral one-time approval).
- **Protocol token persistence**: Protocol permissions granted via on-chain tokens persist across subsequent wallet calls. Re-running the same counterparty + protocol combination does not re-prompt - the existing token satisfies the check.
- **Ephemeral spending does not persist**: When the user chooses "Spend" (ephemeral), each subsequent transaction for the same originator triggers a new spending prompt. Standing authorizations created via "Allow up to" persist as DSAP tokens.
- **No PACT without counterpartyPermissions**: When the manifest does not declare `metanet.counterpartyPermissions`, no PACT prompt is triggered for Level 2 protocols - they fall through to individual protocol access prompts instead.
- **Individual prompts as fallback**: When permissions are not covered by a grouped flow (or the grouped flow has already been satisfied), individual permission prompts are shown sequentially - one per permission type per operation.
- **Concurrent request prompting**: When two createAction calls fire concurrently requiring the same protocol permission, the wallet may prompt for the same protocol twice if the first grant has not yet been cached before the second check runs.
...
This format lacked:
- W3C Web App Manifest compliance (no `short_name`, `icons`, `display`, etc.)
- Schema versioning (no `schemaVersion` field)
- The `name` field was the only metadata used from the manifest, stored at the top level
...
The `fetchManifestPermissions()` method in the wallet-toolbox will be updated to implement this fallback logic. The internal data structures and on-chain token formats are unchanged - only the manifest namespace changes.
...
- **Expiry policy guidance**: The default expiry is `0` (never expires). Guidance on recommended expiry durations for different permission types would improve security posture.
- **PACT persistence**: PACT establishment is currently cached in-memory only. If the wallet restarts, PACT is re-verified by checking for existing protocol tokens. The implications of this for long-lived sessions should be documented.
- **PACT testing coverage**: The reference test application (`peergroup.bsvapp.net`) does not declare `counterpartyPermissions` in its manifest, so the PACT prompt flow (Section 5.6) has not been verified against a live application. A test application that declares `counterpartyPermissions` would enable end-to-end PACT validation.

---

## Appendix A: Manifest Permission Schema

The following JSON Schema defines the structure of `metanet.groupPermissions` and `metanet.counterpartyPermissions` within an application's `manifest.json`.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "Metanet Manifest Permissions",
  "description": "Schema for the metanet property in a Metanet-compatible application manifest.json",
  "type": "object",
  "properties": {
    "schemaVersion": {
      "type": "integer",
    ...
    "groupPermissions": {
      "type": "object",
      "description": "Permissions the application requests as a group on first interaction.",
      "properties": {
        "description": {
          "type": "string",
          "description": "Human-readable description of the permission group."
        },
        "spendingAuthorization": {
          "type": "object",
          "description": "Monthly spending limit request.",
          "properties": {
            "amount": {
              "type": "integer",
              "minimum": 1,
              "description": "Requested monthly spending limit in satoshis."
            },
            "description": {
              "type": "string",
              "description": "Human-readable reason for the spending request."
            }
          },
          "required": ["amount"]
        },
        "protocolPermissions": {
          "type": "array",
          "description": "Protocol access requests.",
          "items": {
            "type": "object",
            "properties": {
              "protocolID": {
                "type": "array",
                "items": [
                  { "type": "integer", "enum": [1, 2], "description": "Security level." },
                  { "type": "string", "minLength": 1, "description": "Protocol name." }
                ],
                "minItems": 2,
                "maxItems": 2,
                "description": "Tuple of [securityLevel, protocolName]."
              },
              "counterparty": {
                "type": "string",
                "description": "Compressed public key of the counterparty. Empty string acts as wildcard (matches any counterparty). Defaults to 'self' if omitted."
              },
              "description": {
                "type": "string",
                "description": "Human-readable description of the protocol usage."
              }
            },
            "required": ["protocolID"]
          }
        ...
        "basketAccess": {
          "type": "array",
          "description": "Basket access requests.",
          "items": {
            "type": "object",
            "properties": {
              "basket": {
              ...
              "description": {
                "type": "string",
                "description": "Human-readable description of the basket usage."
              }
            },
            "required": ["basket"]
          }
        ...
        "certificateAccess": {
          "type": "array",
          "description": "Certificate access requests.",
          "items": {
            "type": "object",
            "properties": {
              "type": {
              ...
              "verifierPublicKey": {
                ...
                "description": "Compressed public key of the verifier."
              },
              "fields": {
                "type": "array",
                "items": { "type": "string" },
                "description": "Specific certificate fields being requested (selective disclosure)."
              },
              "description": {
                "type": "string",
                "description": "Human-readable description of the certificate usage."
              }
            },
            "required": ["type", "verifierPublicKey", "fields"]
          }
        }
      },
    ...
    "counterpartyPermissions": {
      "type": "object",
      "description": "PACT declarations for Level 2 peer interactions.",
      "properties": {
        "description": {
          "type": "string",
          "description": "Human-readable description of the counterparty trust requirements."
        },
        "protocols": {
          "type": "array",
          "description": "Level 2 protocols requiring counterparty trust.",
          "items": {
            "type": "object",
            "properties": {
              "protocolID": {
                "type": "array",
                "items": [
                  { "type": "integer", "const": 2, "description": "Must be Level 2." },
                  { "type": "string", "minLength": 1, "description": "Protocol name." }
                ],
                "minItems": 2,
                "maxItems": 2,
                "description": "Tuple of [2, protocolName]. Only Level 2 is valid."
              },
              "description": {
                "type": "string",
    ...
    "trust": {
      "type": "object",
      "description": "BRC-68 trust entity metadata.",
      "properties": {
        "name": {
          "type": "string",
          "minLength": 5,
        ...
        "icon": {
          "type": "string",
          "format": "uri",
          "description": "URL to entity icon image."
        },
        "publicKey": {
          "type": "string",
          "pattern": "^(02|03)[a-fA-F0-9]{64}$",
          "description": "Entity compressed public key."
        }
      },
      "required": ["name", "icon", "publicKey"]
    }
  },
...
This schema defines the `metanet` property only. A complete `manifest.json` also includes W3C Web App Manifest fields (`name`, `short_name`, `description`, `start_url`, `display`, `icons`, etc.) at the top level.
