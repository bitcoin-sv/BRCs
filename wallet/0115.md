...
- **Security Level**: An integer (0, 1, or 2) that classifies protocol access. **Level 0** is open (no permission required). **Level 1** protocols are application-scoped. **Level 2** protocols are counterparty-specific (a distinct external entity is involved).
...
| `protocols` | array | Array of `{ protocolID: [2, name], description }` - required peer-interaction protocols, **Level 2 only** |
...
PACT is not an in-memory-only permission. A granted PACT is represented by on-chain Level 2 protocol permission tokens scoped to the originator + counterparty pair. In-memory PACT caches are optimization-only and MUST NOT be treated as the source of truth.
...
5. Marks the PACT as established in the in-memory cache for faster subsequent checks.
...
4. If the relevant Level 2 protocols declared in `counterpartyPermissions` have existing on-chain tokens for this counterparty -> `true` (and caches the result).
5. Otherwise -> `false`.
