...
The wallet MUST enforce that all declared `counterpartyPermissions.protocols` entries are Level 2 (`protocolID[0] === 2`). Entries that are not Level 2 are silently dropped.
...
- **`counterparty`** (optional): `'self'`, `'anyone'`, or a compressed public key identifying the other party. Relevant only for Level 2 protocols. Empty string is invalid.
...
- **Protocol**: The request's `protocolID` must exactly match a manifest entry. If the manifest entry includes `counterparty`, it MUST exactly match the request counterparty. If `counterparty` is omitted, matching is by `protocolID` only. Empty-string counterparty values are invalid. **Privileged** protocol requests are never included in grouped flows.
...
2. Filters to Level 2 protocol entries where `counterparty` is omitted or exactly matches the current request's counterparty.
...
2. If `ephemeral !== true`, creates or renews the corresponding on-chain permission token.
        ...
        "protocolPermissions": {
          ...
          "items": {
            ...
            "properties": {
              ...
              "counterparty": {
                "type": "string",
                "pattern": "^(self|anyone|(02|03)[a-fA-F0-9]{64})$",
                "description": "Optional counterparty scope. Must be 'self', 'anyone', or a compressed public key. Empty string is invalid."
              },
              "description": {
                "type": "string",
