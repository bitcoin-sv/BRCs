# BRC-123: BSV Multicast Transaction Frame Format
...
1. Reads 44 bytes (sufficient for legacy header or BRC-123 header start)
2. Inspects byte 6 (Frame Version)
   - **Version 0x01 (legacy):** header complete; read `PayLen` at bytes 40–43
   - **Version 0x02 (BRC-123):** read 48 more bytes; read `PayLen` at bytes 88–91
...
| `FrameVerBRC122`   | 2          | `0x02`       | Current BRC-123 frame version |
| `HeaderSizeLegacy` | 44         | `0x2C`       | Legacy header size in bytes   |
| `HeaderSize`       | 92         | `0x5C`       | BRC-123 header size in bytes  |
```
